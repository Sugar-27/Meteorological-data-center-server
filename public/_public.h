...
void PickNumber(const char* src,
                char* dest,
                const bool bsigned,
                const bool bdot);
...
bool GetXMLBuffer(const char* xmlbuffer,
...
bool AddTime(const char* in_stime,
             char* out_stime,
             const int timetvl,
             const char* fmt = 0);
///////////////////////////////////// /////////////////////////////////////
...
class CDir {
   public:
    ...
    char m_FullFileName
        [301];  // 文件全名，包括目录名，例如：/tmp/root/data.xml。
    ...
    bool _OpenDir(const char* in_DirName,
                  const char* in_MatchStr,
...
bool RENAME(const char* srcfilename,
...
class CFile {
   ...
   public:
    ...
    bool OpenForRename(const char* filename,
                       const char* openmode,
                       bool bEnBuffer = true);
...
class CTcpClient {
   public:
    ...
    bool
        m_btimeout;  // 调用Read方法时，失败的原因是否是超时：true-超时，false-未超时。
